...
function Base (args) {
	...
	if (typeof args == 'string') {
		//CSS模拟
		if(args.indexOf(' ') != -1){
            var elements = args.split(' ');   //把节点分别保存到数组里
            var childElements = [];           //存放临时节点数组,解决被覆盖问题
            var node = [];                    //用来存放父节点用的
...
Base.prototype.text = function (str) {
	for (var i = 0; i < this.elements.length; i++) {
		if (arguments.length == 0) {
		 ...
		 setTnnerText(this.elements[i],text);
	}
	return this;
}
...
Base.prototype.animate = function (obj) {
	for (var i = 0; i < this.elements.length; i++) {
        ...
        if (alter != undefined && target == undefined) {
        	...
        	throw new Ettor('alter增量或target目标量必须传一个');
        }
            ...
            if (attr == 'opacity') {                   
                 if (step == 0){
                	setOpacity();
			 	    ...
			 	    setOpacity();
                  } else if (step < 0 && (parseFloat(getStyle(element,attr)-target))*100  <= Math.abs(step)) {
               	    setOpacity();
                  } else {
